...
int initialize(void)
{
    ...
    const GLchar* vertexShaderSource = readShaderFile("../shaders/main_vs.glsl");
    const GLchar* fragmentShaderSource = readShaderFile("../shaders/main_fs[lambart].glsl");
    ...
    // Load model
    if (!loadModel("model.fbx", &sceneMeshes, &meshCount, 1.0f)) {
