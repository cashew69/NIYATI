...
void renderer(float rotationAngle) {
    ...
    if (mainShaderProgram && mainShaderProgram->id) {
        ...
        vec3 viewPos = vec3(0.0f, 2.0f, 5.0f);
        ...
        for (int i = 0; i < meshCount; i++) {
            mat4 modelMatrix = vmath::rotate(rotationAngle, vec3(0.0f, 1.0f, 0.0f)) * vmath::rotate(-90.0f, vec3(1.0f, 0.0f, 0.0f))  ;
            
            if (modelLoc != -1) glUniformMatrix4fv(modelLoc, 1, GL_FALSE, modelMatrix);
            
            // Use the setMaterialUniforms function you already have!
            ...
            if (sceneMeshes[i].ibo && sceneMeshes[i].indexCount > 0) {
                ...
                glDrawArrays(GL_TRIANGLES, 0, sceneMeshes[i].indexCount);
            }
