...
static SceneNode* wheelModelNode = nullptr;  // EMPTY parent of wheel mesh (id 132)
...
void scene1Init(void)
{
    ...
    targetspline   = spline1;
    krushna        = sg_FindById(103);
    wheelModelNode = sg_FindById(132);
...
void scene1Update(void)
{
    ...
    // Spin wheel around its local Y axis (90 deg/s)
    if (wheelModelNode) {
        wheelModelNode->rotation_euler[1] += g_DeltaTime * 90.0f;
        if (wheelModelNode->rotation_euler[1] > 360.0f)
            wheelModelNode->rotation_euler[1] -= 360.0f;
    }
}
