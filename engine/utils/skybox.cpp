...
unsigned int loadCubemapFromFaces(const char* faces[6])
{
    ...
    for (unsigned int i = 0; i < 6; i++)
    {
        ...
        if (!data) {
            ...
            snprintf(fallbackPath, sizeof(fallbackPath), "../%s", faces[i]);
            data = stbi_load(fallbackPath, &width, &height, &nrChannels, 0);
            if (data) fprintf(gpFile, "Found cubemap face using fallback: %s\n", fallbackPath);
        }
#endif
        if (data)
        {
...
static unsigned int convertHDRToCubemap(const char* hdrPath)
{
    ...
    if (!data) {
        char fallbackPath[1024];
        snprintf(fallbackPath, sizeof(fallbackPath), "../%s", hdrPath);
        data = stbi_loadf(fallbackPath, &width, &height, &nrComponents, 0);
        if (data) fprintf(gpFile, "Found HDR image using fallback: %s\n", fallbackPath);
    }
#endif
...
void initSkybox(const char* hdrPath)
{
    ...
    if (envCubemap)
    {
        generateIBLMaps(envCubemap); // requires pbr.h
